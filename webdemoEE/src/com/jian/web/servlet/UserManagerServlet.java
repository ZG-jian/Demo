...
import com.jian.service.impl.UserManagerServiceImpl;
...
public class UserManagerServlet extends HttpServlet {
    ...
    private void addUser(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException {
        ...
        try {
            ...
            userManagerService.addUsers(users);
            //为了防止表单重复提交的问题，重定向跳转
    ...
    private void modifyUser(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException{
        String userid = req.getParameter("userid");//因为createUser没取userid，要取一下放进去
        //先获取用户提交的数据
        Users users = this.createUsers(req);
        users.setUserid(Integer.parseInt(userid));
        try {
            UserManagerService userManagerService = new UserManagerServiceImpl();
            userManagerService.modifyUser(users);//取了id后传进去调用才能正常的修改
            resp.sendRedirect("ok.jsp");
        }catch (Exception e){
            e.printStackTrace();
            resp.sendRedirect("error.jsp");
 ...
 private void dropUser(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException{
     ...
     try {
         ...
         resp.sendRedirect("ok.jsp");//也是DML操作，用重定向来跳转
     }catch (Exception e){
         e.printStackTrace();
         resp.sendRedirect("error.jsp");
     }
